...
def get_customer_clients(config: dict, session: rq.Session, submanager_id=None) -> list:
    ...
    customer_id = submanager_id if submanager_id is not None else config['google_login_customer_id']
...
def get_custom_columns(config, session, customer_id):
    ...
    """
    url = f"https://searchads360.googleapis.com/v0/customers/{customer_id}/customColumns"
    response = make_sa360_request(
        config,
        method="GET", 
        url=url,
...
def get_custom_column_data(config, session, customer_id, custom_columns, date_cursor):
    ...
    start_date = (datetime.now() - relativedelta(years=2)).date().isoformat() if date_cursor is None else date_cursor
    payload = {
            "query": f"SELECT campaign.id, segments.date, {custom_columns} FROM campaign WHERE segments.date BETWEEN '{start_date}' AND '{datetime.now().date().isoformat()}' ORDER BY segments.date ASC"
        }
    response = make_sa360_request(
        config,
        method="POST",
